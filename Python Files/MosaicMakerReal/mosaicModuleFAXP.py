...
import json, urllib.request, urllib.parse, urllib.error, urllib.request, urllib.error, urllib.parse, io
...
from markup_1_10 import markup   ## ***From http://markup.sourceforge.net/
...
##    print "---------------------------------------------------------------------"
    ...
    while startIndex<int(numberOfResults):
        ...
        try:
            ...
            data = urllib.request.urlopen(SEARCH_URL+'?'+'key='+MY_API_KEY+'&'+'cx='+SEARCH_ENGINE+'&'+'q='+searchQuery+'&'+'searchType='+searchType+'&start='+str(startIndex)+'&'+'imgColorType='+imgColorType)

        except:
            data = urllib.request.urlopen(SEARCH_URL+'?'+'key='+MY_API_KEY_2+'&'+'cx='+SEARCH_ENGINE+'&'+'q='+searchQuery+'&'+'searchType='+searchType+'&start='+str(startIndex)+'&'+'imgColorType='+imgColorType)
...
def getAveRgbArrayWebNew(imageUrlArray, fineness):
    ...
    for item in imageUrlArray:
        try:
            filename=io.StringIO(urllib.request.urlopen(item).read())
